...
public class ConvertFromMockitoWhenIntention extends ConvertStubbingIntentionBase {
    ...
    public boolean isAvailableFor(PsiMethodCallExpression methodCall) {
        return WHEN.equals(getMethodName(methodCall))
            && StubbingApproach.MOCKITO_WHEN.isStubbedBy(methodCall)
    ...
    public List<AnAction> actionSelectionOptions(Editor editor, PsiFile file) {
        ...
        if (!isBDDMockitoEnforced(file) && doAllCallChainsMatch(DOESNT_CONTAIN_THEN, isBulkMode, editor, file)) {
            actions.add(new ConvertStubbingAction(StubbingApproach.MOCKITO_WHEN, StubbingApproach.MOCKITO_DO_X, isBulkMode));
        }
        if (!isMockitoEnforced(file)) {
            actions.add(new ConvertStubbingAction(StubbingApproach.MOCKITO_WHEN, StubbingApproach.BDDMOCKITO_GIVEN, isBulkMode));
            actions.add(new ConvertStubbingAction(StubbingApproach.MOCKITO_WHEN, StubbingApproach.BDDMOCKITO_WILL_X, isBulkMode));
        }
        return actions;
    }
}
